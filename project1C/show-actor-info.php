...
<article>
  <h2>Actor Searching:</h2>
  <form method="GET" action="search.php">
    <input type="text" name="searchcontent" size=100>
    <input type="submit" name="ifactor" value="search">
  </form>
...
$idnumber=$_GET['idnumber'];
 if($idnumber!=NULL){
    ...
    $query="SELECT last, first, sex, dob, dod FROM Actor WHERE id=$idnumber";
    ...
    $rows = $rs->num_rows;
    print '<br>Actor Information: ' . $rs->num_rows;
    echo '<br><table>';
    echo '<tr>';
    ...
    //display this actor's movies
    $query="SELECT id, role, title, year FROM MovieActor, Movie WHERE aid=$idnumber AND mid=id";
    $rs = $db->query($query);
    $colums = $rs->field_count;
    $rows = $rs->num_rows;
